...
fun main() {
    ...
    val bookQueryResult = executeQuery(client, BookQuery(BookQuery.Variables(listOf(1, 2, 3))))
    printQueryResult(bookQueryResult)

    println("*****Simple Courses retrieval example*****")
    val courseQueryResult = executeQuery(client, CourseQuery(CourseQuery.Variables(listOf(1, 2, 3))))
...
fun printQueryResult(queryResult: GraphQLClientResponse<*>): Unit {
    ...
    when (data) {
        is BookQuery.Result? -> data?.searchBooks?.let { books ->
            books.filterNotNull().map { it.title }.forEach(::println)
        }
        
        is CourseQuery.Result? -> data?.searchCourses?.let { courses ->
...
): GraphQLClientResponse<T> = runBlocking {
